...
<head>
    ...
    <title>@yield('title', 'Admin') — BrewHouse</title>
...
{{-- Sidebar --}}
<aside class="sidebar">
    <a href="{{ route('orders.index') }}" class="flex items-center gap-3 mb-8 px-2">
    ...
    <a href="{{ route('orders.queue') }}" class="sidebar-link {{ request()->routeIs('orders.queue') ? 'active' : '' }}">
        <i class="fas fa-list-ol"></i> Queue
        @php $qc = \App\Models\Order::whereIn('status',['queue','preparing'])->count(); @endphp
        @if($qc > 0)
        <span class="ml-auto bg-caramel text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $qc }}</span>
        @endif
    </a>
    ...
    <a href="{{ route('admin.sales') }}" class="sidebar-link {{ request()->routeIs('admin.sales') ? 'active' : '' }}">
        <i class="fas fa-chart-line"></i> Sales
    </a>
    <a href="{{ route('admin.stock') }}" class="sidebar-link {{ request()->routeIs('admin.stock') ? 'active' : '' }}">
        @php $ls = \App\Models\Product::whereRaw('stock <= low_stock_threshold')->whereNull('deleted_at')->count(); @endphp
        <i class="fas fa-boxes-stacked"></i> Stock
        @if($ls > 0)
        <span class="ml-auto bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $ls }}</span>
        @endif
    </a>
    ...
    <a href="{{ route('admin.todos.index') }}" class="sidebar-link {{ request()->routeIs('admin.todos.*') ? 'active' : '' }}">
        @php $tc = \App\Models\Todo::whereIn('priority',['urgent','high'])->where('status','!=','completed')->count(); @endphp
        <i class="fas fa-check-square"></i> To-Do
        @if($tc > 0)
        <span class="ml-auto bg-orange-400 text-white text-xs font-bold px-2 py-0.5 rounded-full">{{ $tc }}</span>
        @endif
    </a>
</aside>
...
<div class="main-content">
    ...
    <div class="bg-white border-b border-gray-100 px-8 py-4 flex items-center justify-between sticky top-0 z-20">
        <div>
            <h1 class="font-display text-xl text-gray-800">@yield('page-title', 'Admin')</h1>
    ...
    <div class="mx-8 mt-4 bg-red-50 border border-red-200 text-red-800 px-5 py-3 rounded-xl animate-in">
        ...
        <div class="flex items-center gap-2"><i class="fas fa-exclamation-circle text-red-400"></i> {{ $err }}</div>
        @endforeach
    </div>
    @endif

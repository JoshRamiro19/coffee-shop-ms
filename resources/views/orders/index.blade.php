@extends('layouts.app')
@section('title', 'Orders — BrewHouse')
...
<div class="flex gap-6">
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="mb-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                ...
                <div class="card p-4 border-l-4 {{ $order->status === 'preparing' ? 'border-orange-400' : ($order->status === 'queue' ? 'border-blue-400' : 'border-yellow-400') }}">
                    <div class="flex justify-between items-start mb-2">
                        <div>
                            <p class="font-semibold text-espresso">{{ $order->customer_name }}</p>
                    ...
                    <div class="flex items-center justify-between border-t pt-2">
                        <span class="font-bold text-espresso">₱{{ number_format($order->total_amount, 2) }}</span>
                        <div class="flex gap-2">
                            <select onchange="updateStatus({{ $order->id }}, this.value)" class="text-xs border rounded-lg px-2 py-1 text-gray-600">
                                <option value="pending"   {{ $order->status === 'pending'   ? 'selected' : '' }}>Pending</option>
                                <option value="queue"     {{ $order->status === 'queue'     ? 'selected' : '' }}>Queue</option>
                                <option value="preparing" {{ $order->status === 'preparing' ? 'selected' : '' }}>Preparing</option>
                            </select>
                            <button onclick="cancelOrder({{ $order->id }})" class="text-red-400 hover:text-red-600 text-xs px-2 py-1 border border-red-200 rounded-lg">Cancel</button>
                        </div>
                    </div>
                </div>
                @endforeach
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-gray-50 hover:bg-foam transition-colors">
                            <td class="px-4 py-3 font-mono text-xs text-gray-400">{{ $order->order_number }}</td>
                            <td class="px-4 py-3 font-medium text-espresso">{{ $order->customer_name }}</td>
                            <td class="px-4 py-3 text-gray-500">
                                {{ $order->items->sum('quantity') }} item(s):
                                {{ $order->items->map(fn($i) => $i->product->name)->implode(', ') }}
                            </td>
                            <td class="px-4 py-3 text-right font-semibold text-espresso">₱{{ number_format($order->total_amount, 2) }}</td>
                            <td class="px-4 py-3 text-gray-400">{{ $order->completed_at?->format('M d, g:ia') }}</td>
    ...
    <div class="w-72 shrink-0">
        <div class="card p-4 sticky top-24">
            ...
            <div class="mb-4">
                <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">{{ ucfirst($category) }}</p>
                @foreach($items as $product)
                <div class="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div>
                        <p class="text-sm font-medium text-espresso">{{ $product->name }}</p>
                        <p class="text-xs text-gray-400">Stock: {{ $product->stock }}</p>
                    </div>
                    <span class="text-sm font-semibold text-caramel">₱{{ number_format($product->price, 2) }}</span>
                </div>
                @endforeach
            </div>
            @endforeach
        ...
        <div id="step2" class="hidden flex flex-col h-full overflow-hidden">
            ...
            <div class="flex gap-4 p-6 overflow-hidden" style="min-height:0;flex:1">
                ...
                <div class="flex-1 overflow-y-auto pr-2">
                    @foreach($products as $category => $items)
                    <div class="mb-4">
                        <p class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 sticky top-0 bg-white py-1">{{ ucfirst($category) }}</p>
                        @foreach($items as $product)
                        <div class="flex items-center justify-between p-3 rounded-xl hover:bg-foam cursor-pointer border border-transparent hover:border-caramel/20 transition-all mb-1"
                            ...
                            <div>
                                <p class="font-medium text-espresso text-sm">{{ $product->name }}</p>
                                <p class="text-xs text-gray-400">Stock: {{ $product->stock }}</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-caramel font-semibold text-sm">₱{{ number_format($product->price, 2) }}</span>
